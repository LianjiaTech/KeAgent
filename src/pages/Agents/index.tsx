...
import { AlertCircle, Bot, Check, Plus, RefreshCw, Search, Settings2, Trash2, X } from 'lucide-react';
...
interface AgentTemplateCategory {
  id: string;
  name_zh: string;
  name_en: string;
...
function AddAgentDialog({
    ...
    const fetchTemplates = async () => {
      try {
        const [categoriesRes, templatesRes] = await Promise.all([
          hostApiFetch<{ success: boolean; categories: AgentTemplateCategory[] }>('/api/agent-templates/categories'),
          hostApiFetch<{ success: boolean; templates: AgentTemplate[] }>('/api/agent-templates'),
        ]);
  ...
  const templatesByCategory = useMemo(() => {
    ...
    for (const template of templates) {
      const catId = template.categoryId || 'other';
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      await onCreate(name.trim(), selectedTemplate?.id);
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <Card className="w-full max-w-3xl max-h-[85vh] flex flex-col rounded-3xl border-0 shadow-2xl bg-[#f3f1e9] dark:bg-card overflow-hidden">
        ...
        <CardContent className="space-y-4 pt-2 overflow-hidden flex flex-col flex-1 p-6">
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'templates' | 'custom')} className="flex-1 flex flex-col overflow-hidden">
            <TabsList className="grid w-full grid-cols-2 mb-4 shrink-0">
              <TabsTrigger value="templates" className="text-[13px]">{t('createDialog.fromTemplate', '从模板创建')}</TabsTrigger>
              <TabsTrigger value="custom" className="text-[13px]">{t('createDialog.custom', '自定义创建')}</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="templates" className="flex-1 overflow-hidden flex flex-col mt-0">
              {loading ? (
                ...
                <div className="flex gap-4 flex-1 overflow-hidden">
                  {/* Category List */}
                  <div className="w-56 shrink-0 border-r border-black/10 dark:border-white/10 pr-4">
                    <div className="h-[350px] overflow-y-auto">
                      <div className="space-y-1">
                        ...
                        >
                          {t('createDialog.allTemplates', '全部模板')}
                          <span className="ml-2 text-[11px] text-muted-foreground">({templates.length})</span>
                        </button>
                        {categories.map((category) => {
                          const count = templatesByCategory.get(category.id)?.length || 0;
                          return (
                            <button
                              key={category.id}
                    ...
                    <div className="mb-3 shrink-0">
                      <div className="relative">
                        ...
                        <Input
                          ...
                          placeholder={t('createDialog.searchTemplates', '搜索模板...')}
                          className="pl-9 h-9 text-[13px] bg-white/50 dark:bg-black/20"
                        />
                      </div>
                    </div>
                    <div className="flex-1 overflow-y-auto">
                      <div className="grid grid-cols-2 gap-2 pr-2">
                          ...
                          >
                            <div className="flex items-start justify-between gap-2">
                              <div className="min-w-0 flex-1">
                                ...
                                {template.category && (
                                  <p className="text-[11px] text-muted-foreground mt-0.5 truncate">{template.category.name_zh}</p>
                                )}
                              </div>
                              {selectedTemplate?.id === template.id && (
                        ...
                        {filteredTemplates.length === 0 && (
                          <div className="col-span-2 py-8 text-center text-muted-foreground text-[13px]">
                            {t('createDialog.noTemplatesFound', '没有找到匹配的模板')}
                          </div>
                        )}
                      </div>
                    </div>
            ...
            <TabsContent value="custom" className="flex-1 mt-0">
              <div className="py-4">
                <p className="text-[14px] text-muted-foreground mb-4">
                  {t('createDialog.customDescription', '创建一个空白 Agent，您可以稍后自定义其行为。')}
                </p>
              </div>
            </TabsContent>
          </Tabs>
          ...
          <div className="border-t border-black/10 dark:border-white/10 pt-4 shrink-0">
