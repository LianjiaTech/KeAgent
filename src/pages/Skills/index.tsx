...
export function Skills() {
  ...
  const [selectedSource, setSelectedSource] = useState<
    'all' | 'built-in' | 'marketplace' | 'real-estate'
  >('all');

  const REAL_ESTATE_SKILL_PREFIX = 'floorplan-';
  ...
  const filteredSkills = safeSkills
    .filter((skill) => {
      ...
      if (selectedSource === 'built-in') {
        ...
        matchesSource = !skill.isBundled;
      } else if (selectedSource === 'real-estate') {
        matchesSource = skill.id.startsWith(REAL_ESTATE_SKILL_PREFIX);
      }
  ...
  const sourceStats = {
    ...
    realEstate: safeSkills.filter((s) => s.id.startsWith(REAL_ESTATE_SKILL_PREFIX)).length,
  };
  ...
  const [skillsDirPath, setSkillsDirPath] = useState('~/.keagent/skills');
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-black/10 dark:border-white/10 pb-4 mb-4 shrink-0 gap-4">
          <div className="flex items-center flex-wrap gap-4 text-[14px]">
            ...
            <div className="flex items-center gap-6">
              ...
              <button
                onClick={() => setSelectedSource('real-estate')}
                className={cn(
                  'font-medium transition-colors flex items-center gap-1.5',
                  selectedSource === 'real-estate'
                    ? 'text-foreground'
                    : 'text-muted-foreground hover:text-foreground'
                )}
              >
                {t('filter.realEstate', { count: sourceStats.realEstate })}
              </button>
            </div>
          </div>
