...
export function Settings() {
  ...
  const handleRunOpenClawDoctor = async (mode: 'diagnose' | 'fix') => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(
          mode === 'fix' ? t('developer.doctorFixSucceeded') : t('developer.doctorSucceeded')
        );
      } else {
        toast.error(
          result.error ||
            (mode === 'fix' ? t('developer.doctorFixFailed') : t('developer.doctorFailed'))
        );
      }
    } catch (error) {
      const message =
        toUserMessage(error) ||
        (mode === 'fix' ? t('developer.doctorFixRunFailed') : t('developer.doctorRunFailed'));
      toast.error(message);
      setDoctorResult({
        mode,
        success: false,
                ...
                <div className="space-y-8">
                  ...
                  <div className="space-y-4">
                    ...
                    {proxyEnabledDraft && (
                      <div className="space-y-4 pt-2">
                        ...
                        <div className="space-y-2">
                          <Label htmlFor="proxy-bypass" className="text-[13px] text-foreground/80">
                  ...
                  {showCliTools && (
                    <div className="space-y-3">
                      ...
                      <p className="text-[13px] text-muted-foreground">{t('developer.cliDesc')}</p>
                      {isWindows && (
                        <p className="text-[12px] text-muted-foreground">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        >
                          <RefreshCw
                            className={`h-4 w-4 mr-2${doctorRunningMode === 'fix' ? ' animate-spin' : ''}`}
                          />
            ...
            <div className="space-y-3 text-[14px] text-muted-foreground">
              ...
              <div className="flex gap-4 pt-3">
                <Button
                  ...
                  onClick={() => window.electron.openExternal('https://speechhub.ke.com')}
                >
                  {t('about.docs')}
                </Button>
                <Button
                ...
                <Button
                  ...
                  onClick={() =>
                    window.electron.openExternal(
                      'https://beike.feishu.cn/wiki/V6HQwFM4fitk1zkaHJfc5Z6Enlf?from=from_copylink'
                    )
                  }
                >
                  {t('about.faq')}
