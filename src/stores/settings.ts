...
interface SettingsState {
  ...
  setXclaudeServerUrl: (url: string) => void;
...
const defaultSettings = {
  ...
  xclaudeServerUrl: XCLAUDE_SERVER_URL_FROM_ENV,
  setupComplete: false,
};
...
export const useSettingsStore = create<SettingsState>()(
  persist(
    (set) => ({
      ...
      setXclaudeServerUrl: (xclaudeServerUrl: string) => set({ xclaudeServerUrl }),
      markSetupComplete: () => set({ setupComplete: true }),
      resetSettings: () => set(defaultSettings),
    }),
    {
      name: 'clawx-settings',
      // Always override xclaudeServerUrl with the build-time env value so that
      // updating VITE_XCLAUDE_SERVER_URL in .env takes effect on next launch.
