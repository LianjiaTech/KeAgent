...
# Post-installation script for KeAgent on Linux
...
# Create symbolic link for KeAgent app binary
if [ -x /opt/KeAgent/keagent ]; then
    ln -sf /opt/KeAgent/keagent /usr/local/bin/keagent 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/KeAgent/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/KeAgent/chrome-sandbox' || true
else
    chmod 0755 '/opt/KeAgent/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/KeAgent/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/keagent'
...
echo "KeAgent has been installed successfully."
