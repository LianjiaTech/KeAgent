...
<p align="center">
  <a href="#功能特性">功能特性</a> •
  <a href="#为什么选择-KeAgent">为什么选择 KeAgent</a> •
  <a href="#快速上手">快速上手</a> •
  <a href="#系统架构">系统架构</a> •
  <a href="#开发指南">开发指南</a> •
  <a href="#参与贡献">参与贡献</a> •
...
**KeAgent** 是由贝壳找房开源的 AI 智能体桌面客户端，连接强大 AI 智能体与普通用户，将命令行式 AI 编排转变为易用、美观的桌面体验。

- 🖥️ **零门槛上手** — 无需终端，全程图形界面完成 AI 智能体的安装与配置
- 🤖 **多智能体管理** — 支持创建、切换多个 Agent，工作区彼此隔离、独立运行
- 📡 **多频道接入** — 同时连接 Telegram、微信等通讯渠道，每个频道独立绑定 Agent
- ⏰ **定时任务调度** — 可视化配置 AI 定时任务，支持外部投递，让 Agent 7×24 小时工作
- 🧩 **技能市场** — 内置技能面板，一键浏览、安装、管理扩展技能，无需包管理器
- 🔐 **安全凭证存储** — API 密钥存储于系统原生密钥链，支持多供应商（OpenAI、Anthropic 等）
- 🌙 **自适应主题** — 支持浅色 / 深色模式，跟随系统自动切换
- 🌐 **多语言 & 代理** — 原生支持 Windows / macOS，内置多语言与代理设置

---

## 💬 微信交流群

微信账号：**wantongtang** / **Thinkre**（添加好友后拉你入群）
...
构建 AI 智能体不应该需要精通命令行。KeAgent 的设计理念很简单：**强大的技术值得拥有一个尊重用户时间的界面。**
...
当你使用 `@agent` 选择其他智能体时，KeAgent 会直接切换到该智能体自己的对话上下文，而不是经过默认智能体转发。各 Agent 工作区默认彼此分离，但更强的运行时隔离可通过 sandbox 配置。
...
KeAgent 现在还内置了腾讯官方个人微信渠道插件，可直接在 Channels 页面通过内置二维码流程完成微信连接。
...
KeAgent 还会内置预装完整的文档处理技能（`pdf`、`xlsx`、`docx`、`pptx`），在启动时自动部署到托管技能目录（默认 `~/.openclaw/skills`），并在首次安装时默认启用。额外预装技能（`find-skills`、`self-improving-agent`、`tavily-search`、`brave-web-search`）也会默认启用；若缺少必需的 API Key，OpenClaw 会在运行时给出配置错误提示。  
Skills 页面可展示来自多个技能库（托管目录、workspace、额外技能目录），并显示每个技能的实际路径，便于直接打开真实安装位置。
...
### 🌙 自适应主题
支持浅色模式、深色模式或跟随系统主题。KeAgent 自动适应你的偏好设置。

### 🚀 开机启动控制
在 **设置 → 通用** 中，你可以开启 **开机自动启动**，让 KeAgent 在系统登录后自动启动。
...
从 [Releases](https://github.com/wantt/KeAgent/releases) 页面下载适用于你平台的最新版本安装包：
- **macOS**: `.dmg` 文件
- **Windows**: `.exe` 文件
...
首次启动 KeAgent 时，**设置向导** 将引导你完成以下步骤：
...
KeAgent 内置了代理设置，适用于需要通过本地代理客户端访问外网的场景，包括 Electron 本身、OpenClaw Gateway，以及 Telegram 这类频道的联网请求。
...
- 当 KeAgent 代理处于关闭状态时，Gateway 的常规重启会保留已有的 Telegram 频道代理配置。

---

## 🏗️ 系统架构

KeAgent 采用 **双进程 + Host API 统一接入架构**。渲染进程只调用统一客户端抽象，协议选择与进程生命周期由 Electron 主进程统一管理：

<img src="resources/architecture-diagram.svg" alt="KeAgent 架构图" width="100%"/>
### 设计原则
...
- KeAgent 基于 Electron，**单个应用实例出现多个系统进程是正常现象**（main/renderer/zygote/utility）。
...
- 点击窗口关闭按钮（`X`）默认只是最小化到托盘，并不会完全退出应用。请在托盘菜单中选择 **Quit KeAgent** 执行完整退出。
...
我们欢迎社区的各种贡献！无论是修复 Bug、开发新功能、改进文档还是翻译——每一份贡献都让 KeAgent 变得更好。
...
- [ ] **Linux 支持** - 添加 Ubuntu 20.04+ 的原生支持
- [ ] **语音交互** - 集成语音识别（STT）和语音合成（TTS）功能，实现真正的语音对话
- [ ] **多模态支持** - 支持图像、音频等多媒体内容的理解和生成
- [ ] **Agent 协作网络** - 多个 Agent 之间协同工作，自动分解复杂任务
- [ ] **RAG 增强** - 集成向量数据库，支持基于知识库的问答
- [ ] **代码编辑器集成** - 内置代码编辑和预览功能
- [ ] **自定义工作流** - 可视化拖拽方式创建工作流
- [ ] **离线模式** - 支持本地运行小型模型，无需网络连接
- [ ] **插件市场** - 在线浏览和一键安装第三方插件
- [ ] **团队协作** - 共享 Agent 配置和工作空间
...
KeAgent 基于 [GPL-3.0-or-later 许可证](LICENSE) 发布。
