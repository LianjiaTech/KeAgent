# KeAgent Application Installation Instructions

## Two Ways to Install

### Option 1: Using the Graphical DMG (Current Distribution)

1. Double-click the `KeAgent-*.dmg` file
2. Drag "KeAgent.app" to the "Applications" folder (blue folder)
...
3. Scroll to find the warning about KeAgent at the bottom
4. Press "Allow Anyway" next to the KeAgent entry
...
1. Download both the `KeAgent-*.dmg` file and the `install-keagent.sh` script to your computer
...
- If you continue to get warnings after trying the above steps, it may be a certificate trust issue that can be resolved with:
  - `xattr -rd com.apple.quarantine /Applications/KeAgent.app` (run in Terminal)

- If the application fails to run, verify it's in your /Applications folder
- For technical problems, visit [KeAgent Support](https://keagent.ai/troubleshooting) (actual link would be provided as relevant by the team)