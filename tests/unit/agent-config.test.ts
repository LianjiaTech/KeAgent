...
async function writeOpenClawJson(config: unknown): Promise<void> {
  const openclawDir = join(testHome, '.keagent');
  await mkdir(openclawDir, { recursive: true });
  await writeFile(join(openclawDir, 'openclaw.json'), JSON.stringify(config, null, 2), 'utf8');
}

async function readOpenClawJson(): Promise<Record<string, unknown>> {
  const content = await readFile(join(testHome, '.keagent', 'openclaw.json'), 'utf8');
...
describe('agent config lifecycle', () => {
  ...
  it('falls back to the implicit main agent when no list exists', async () => {
    ...
    await expect(listConfiguredAgentIds()).resolves.toEqual(['ceo']);
  });
  ...
  it('deletes the config entry, bindings, runtime directory, and managed workspace for a removed agent', async () => {
      ...
      agents: {
        ...
        list: [
          ...
          },
          {
            id: 'test2',
            name: 'test2',
            workspace: '~/.keagent/workspace-test2',
            agentDir: '~/.keagent/agents/test2/agent',
          },
          {
            id: 'test3',
            name: 'test3',
            workspace: '~/.keagent/workspace-test3',
            agentDir: '~/.keagent/agents/test3/agent',
          },
        ],
      },
      channels: {
    ...
    const test2RuntimeDir = join(testHome, '.keagent', 'agents', 'test2');
  ...
  it('preserves unmanaged custom workspaces when deleting an agent', async () => {
    ...
    await mkdir(join(testHome, '.keagent', 'agents', 'test2', 'agent'), { recursive: true });
