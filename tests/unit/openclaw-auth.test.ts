...
async function readAuthProfiles(agentId: string): Promise<Record<string, unknown>> {
  const content = await readFile(join(testHome, '.keagent', 'agents', agentId, 'agent', 'auth-profiles.json'), 'utf8');
...
describe('saveProviderKeyToOpenClaw', () => {
  ...
  it('only syncs auth profiles for configured agents', async () => {
    ...
    await writeFile(
      join(testHome, '.keagent', 'agents', 'test2', 'agent', 'auth-profiles.json'),
      JSON.stringify({
        version: 1,
        profiles: {
          'legacy:default': {
