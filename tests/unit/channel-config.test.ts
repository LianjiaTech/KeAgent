...
describe('WeChat dangling plugin cleanup', () => {
  ...
  it('removes dangling openclaw-weixin plugin registration and state when no channel config exists', async () => {
    ...
    const staleStateDir = join(testHome, '.keagent', 'openclaw-weixin', 'accounts');
    ...
    await writeFile(join(testHome, '.keagent', 'openclaw-weixin', 'accounts.json'), JSON.stringify(['bot-im-bot']), 'utf8');
    ...
    expect(existsSync(join(testHome, '.keagent', 'openclaw-weixin'))).toBe(false);
  });
});
