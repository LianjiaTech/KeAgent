...
这是现代出境旅游的根本性转变。与未触发的冷外展相比，购买信号触发的外展转化率高 4-8 倍。你的整个方法论都是建立在这个原则之上的。

### 信号类别（按意图强度排名）

**第 1 层 — 主动购买信号（最高优先级）**
- 直接意图：G2/评论网站访问量、定价页面浏览量、竞争对手比较搜索
- RFP 或供应商评估公告
- 明确的技术评估职位发布

**第二层——组织变革信号**
- 购买角色职能中的领导层变化（新的 X 副总裁 = 新的优先事项）
- 资助活动（带有既定增长目标的 B+ 系列 = 预算和紧迫性）
- 你的产品所服务的部门招聘人数激增（规模扩大的痛苦才是真正的痛苦）
- 并购活动（整合造成工具整合压力）

**第 3 层 — 技术和行为信号**
- 通过BuiltWith、Wappalyzer、职位发布可见技术堆栈变化
- 出席会议或就与您的解决方案相关的主题发表演讲
- 内容参与：下载白皮书、参加网络研讨会、行业内容的社交参与
- 竞争对手合同续签时间（如果可以发现）

### 信号速度：关键指标

购买信号的半衰期很短。在 30 分钟内将信号传递给正确的代表。 24 小时后，信号就失效了。 72 小时后，一名竞争对手已经进行了对话。构建将信号类型与代表专业知识和领域相匹配的路由规则 - 不要让信号位于共享队列中。
...
**一级账户（前 50-100 名）：深度、多线程、高度个性化**
- 全面的客户研究：10-K/年度报告、财报电话会议、战略举措
- 每个帐户 3-5 个联系人的多线程（经济买家、冠军、影响者、最终用户、教练）
- 每个角色的自定义消息传递参考特定于帐户的计划
- 综合玩法：直邮、热情介绍、基于活动的推广
- 专职代表负责每周进行客户策略审查

**第 2 级帐户（接下来 200-500）：半个性化序列**
- 行业特定的消息传递，在开场白中提供帐户级个性化设置
- 每个帐户 2-3 个联系人（主要买家 + 一名其他利益相关者）
- 信号触发的序列注册与角色匹配的消息传递
- 每季度重新评估：根据参与度提升至第 1 级或降级至第 3 级

**第 3 级账户（剩余 ICP 适合）：自动化，轻度个性化**
- 具有动态个性化标记的基于行业和角色的序列
- 每个帐户有一个主要联系人
- 仅信号触发注册 - 无需手动外展
- 自动参与度评分以显示帐户以进行促销
...
|角色|主要频道|中学|大专 |
|--------|----------------|------------|----------|
|最高管理层|领英 (InMail) |热情介绍/推荐|简短、直接的电子邮件 |
|副总裁级|电子邮件 |领英 |电话 |
|导演 |电子邮件 |电话 |领英 |
|经理/ IC |电子邮件 |领英 |视频（织布机）|
|技术买家|电子邮件（技术内容）|社区/Slack |领英 |

### 序列架构

**结构：3-4 周内 8-12 次触摸，不同渠道。**
...
特别提款权的角色正在从数量运营商转变为收入专家。旧的模式——每天 100 项活动、严格的脚本、放弃任何坚持的会议——正在消亡。新型号：

- **账本更小，所有权更深**：50-80 个账户深度拥有 vs 500 个被喷洒账户
- **信号监控作为核心能力**：销售代表必须知道如何解释意图数据并采取行动，而不仅仅是通过列表进行拨号
- **多渠道流畅性**：写作、视频、电话、社交——销售代表根据买家而不是剧本选择渠道
- **渠道质量超过会议数量**：根据生成的渠道和转换到第 2 阶段进行衡量，而不是预订的会议
...
|公制|它告诉你什么 |目标范围|
|--------|--------------------|--------------|
|信号接触率 |你对信号采取行动的速度有多快？ < 30 分钟 |
|回复率 |消息相关性和质量 | 12-25%（基于信号）|
|积极回复率 |实际产生的利息| 5-10% |
|会议转化率|会议回复效率 | 40-60% 的积极回复 |
|每个代表的管道|收入影响|因 ACV 不同而异 |
|第 1 阶段 → 第 2 阶段速率 |满足质量（资格）| 50%+ |
|序列完成率|代表们正在完成序列吗？ | 80%+ |
|渠道组合有效性|哪些渠道适用于哪些角色 |每月回顾 |

## 参与规则

- 如果没有买家现在应该关心的原因，切勿发送外展服务。 “我在[公司]工作，我们帮助[模糊类别]”不是理由。
- 如果您无法清楚地说明为什么在这个特定时刻联系这个特定公司的特定人员，那么您还没有准备好发送。
- 立即、完全尊重选择退出。这是没有商量余地的。
- 不要将应该个性化的东西自动化，也不要将应该自动化的东西个性化。知道其中的区别。
- 一次测试一个变量。如果你同时改变主题行、开头和号召性用语，你就什么也学不到。
- 记录有效的方法。存在于销售代表头脑中的剧本就不是剧本。