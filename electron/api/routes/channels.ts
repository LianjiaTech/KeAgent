...
import {
  ...
  installFeishuPluginViaLarkTools,
...
import { feishuLoginManager, type FeishuDomain } from '../../utils/feishu-login';
...
async function awaitWeChatQrLogin(
  ...
  try {
    ...
    scheduleGatewayChannelSaveRefresh(
      ctx,
      OPENCLAW_WECHAT_CHANNEL_TYPE,
      `wechat:loginSuccess:${normalizedAccountId}`
    );
...
async function ensureScopedChannelBinding(channelType: string, accountId?: string): Promise<void> {
  ...
  if (accountId === 'default') {
    if (agents.agents.some((entry) => entry.id === 'ceo')) {
      await assignChannelAccountToAgent('ceo', storedChannelType, 'default');
    }
    return;
  }
...
async function buildChannelAccountsView(ctx: HostApiContext): Promise<ChannelAccountsView[]> {
  const [configuredChannels, configuredAccounts, openClawConfig, agentsSnapshot] =
    await Promise.all([
      listConfiguredChannels(),
      listConfiguredChannelAccounts(),
      readOpenClawConfig(),
  ...
  try {
    gatewayStatus = await ctx.gatewayManager.rpc<GatewayChannelStatusPayload>('channels.status', {
      probe: true,
    });
  } catch {
    gatewayStatus = null;
  ...
  for (const rawChannelType of channelTypes) {
    ...
    const hasLocalConfig =
      configuredChannels.includes(rawChannelType) || Boolean(configuredAccounts[rawChannelType]);
    const channelSection = openClawConfig.channels?.[rawChannelType];
    const channelSummary =
      (gatewayStatus?.channels?.[rawChannelType] as
    ...
    const defaultAccountId =
      configuredAccounts[rawChannelType]?.defaultAccountId ??
    ...
    const accountIds = Array.from(
      new Set([...channelAccountsFromConfig, ...runtimeAccountIds, defaultAccountId])
    );

    const accounts: ChannelAccountView[] = accountIds
      .map((accountId) => {
...
export async function handleChannelRoutes(
  ...
  if (url.pathname === '/api/channels/configured' && req.method === 'GET') {
    ...
    sendJson(res, 200, {
      success: true,
      channels: Array.from(new Set(channels.map((channel) => toUiChannelType(channel)))),
    });
    return true;
  }
  ...
  if (url.pathname === '/api/channels/default-account' && req.method === 'PUT') {
    try {
      ...
      scheduleGatewayChannelSaveRefresh(
        ctx,
        body.channelType,
        `channel:setDefaultAccount:${body.channelType}`
      );
  ...
  if (url.pathname === '/api/channels/binding' && req.method === 'PUT') {
    try {
      const body = await parseJsonBody<{ channelType: string; accountId: string; agentId: string }>(
        req
      );
      await assignChannelAccountToAgent(
        body.agentId,
        resolveStoredChannelType(body.channelType),
        body.accountId
      );
      scheduleGatewayChannelSaveRefresh(
        ctx,
        body.channelType,
        `channel:setBinding:${body.channelType}`
      );
      sendJson(res, 200, { success: true });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
  ...
  if (url.pathname === '/api/channels/binding' && req.method === 'DELETE') {
    try {
      ...
      scheduleGatewayChannelSaveRefresh(
        ctx,
        body.channelType,
        `channel:clearBinding:${body.channelType}`
      );
      sendJson(res, 200, { success: true });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
  ...
  if (url.pathname === '/api/channels/credentials/validate' && req.method === 'POST') {
    try {
      const body = await parseJsonBody<{ channelType: string; config: Record<string, string> }>(
        req
      );
      sendJson(res, 200, {
        success: true,
  ...
  if (url.pathname === '/api/channels/feishu/start-qr' && req.method === 'POST') {
    try {
      const body = await parseJsonBody<{ domain?: FeishuDomain }>(req);
      await feishuLoginManager.start(body.domain ?? 'feishu');
      sendJson(res, 200, { success: true });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
    }
  ...
  if (url.pathname === '/api/channels/feishu/cancel-qr' && req.method === 'POST') {
    try {
  ...
  if (url.pathname === '/api/channels/wechat/start' && req.method === 'POST') {
    try {
      ...
      const loginKey = setActiveQrLogin(
        UI_WECHAT_CHANNEL_TYPE,
  ...
  if (url.pathname === '/api/channels/config' && req.method === 'POST') {
    try {
      ...
      if (storedChannelType === 'feishu') {
        let installResult = ensureFeishuPluginInstalled();
        if (!installResult.installed) {
          // Bundled plugin not available — try the official lark-tools CLI
          const appId = body.config['appId'] as string | undefined;
          const appSecret = body.config['appSecret'] as string | undefined;
          installResult = await installFeishuPluginViaLarkTools(
            appId && appSecret ? { appId, appSecret } : undefined
          );
        }
        if (!installResult.installed) {
          sendJson(res, 500, {
      ...
      scheduleGatewayChannelSaveRefresh(
        ctx,
        storedChannelType,
        `channel:saveConfig:${storedChannelType}`
      );
      sendJson(res, 200, { success: true });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
  ...
  if (url.pathname === '/api/channels/config/enabled' && req.method === 'PUT') {
    try {
      ...
      scheduleGatewayChannelRestart(
        ctx,
        `channel:setEnabled:${resolveStoredChannelType(body.channelType)}`
      );
      sendJson(res, 200, { success: true });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
    ...
    try {
      ...
      if (accountId) {
        ...
        scheduleGatewayChannelSaveRefresh(
          ctx,
          storedChannelType,
          `channel:deleteAccount:${storedChannelType}`
        );
      } else {
