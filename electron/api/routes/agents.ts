...
export async function handleAgentRoutes(
  ...
  if (url.pathname === '/api/agents' && req.method === 'POST') {
    try {
      const body = await parseJsonBody<{ name: string; templateId?: string }>(req);

      // If templateId is provided, create agent from template
      if (body.templateId) {
        const template = await getAgentTemplate(body.templateId);
        if (!template) {
          sendJson(res, 404, { success: false, error: 'Template not found' });
          return true;
        }
        const snapshot = await createAgentFromTemplate(body.name, template);
        syncAllProviderAuthToRuntime().catch((err) => {
          console.warn('[agents] Failed to sync provider auth after agent creation:', err);
        });
        scheduleGatewayReload(ctx, 'create-agent-from-template');
        sendJson(res, 200, { success: true, ...snapshot });
        return true;
      }

      // Regular agent creation without template
  ...
  if (url.pathname === '/api/agent-templates/categories' && req.method === 'GET') {
    try {
  ...
  if (url.pathname === '/api/agent-templates' && req.method === 'GET') {
    try {
  ...
  if (url.pathname.startsWith('/api/agent-templates/') && req.method === 'GET') {
    try {
      const templateId = decodeURIComponent(url.pathname.slice('/api/agent-templates/'.length));
