...
export function registerIpcHandlers(
  ...
  // Feishu / Lark QR login handlers
  registerFeishuQrHandlers(mainWindow);
...
function registerHostApiProxyHandlers(): void {
  ...
  // Xclaude external fetch — proxies full URLs to bypass renderer CORS
  ipcMain.handle(
    'xclaude:fetch',
    async (
      _,
      ...
      try {
        ...
        if (!url.startsWith('http://') && !url.startsWith('https://')) {
          throw new Error(`Invalid xclaude fetch URL: ${url}`);
        }
        const method = (request.method || 'GET').toUpperCase();
        const headers: Record<string, string> = { ...(request.headers || {}) };
        const body = request.body;

        const response = await proxyAwareFetch(url, { method, headers, body });
 ...
 * Direct read/write to ~/.keagent/openclaw.json (bypasses Gateway RPC)
 */
...
function registerOpenClawHandlers(gatewayManager: GatewayManager): void {
  ...
  // Get the OpenClaw config directory (~/.keagent)
  ipcMain.handle('openclaw:getConfigDir', () => {
    return getOpenClawConfigDir();
  });

  // Get the OpenClaw skills directory (~/.keagent/skills)
 ...
 * Feishu / Lark QR Login Handlers
 */
function registerFeishuQrHandlers(mainWindow: BrowserWindow): void {
  // Start the Feishu device-flow QR login
  ipcMain.handle('channel:requestFeishuQr', async (_, domain: FeishuDomain = 'feishu') => {
    try {
      logger.info('channel:requestFeishuQr', { domain });
      await feishuLoginManager.start(domain);
      return { success: true };
    } catch (error) {
      logger.error('channel:requestFeishuQr failed', error);
      return { success: false, error: String(error) };
    }
  });

  // Cancel an ongoing Feishu QR login
  ipcMain.handle('channel:cancelFeishuQr', async () => {
    try {
      await feishuLoginManager.stop();
      return { success: true };
    } catch (error) {
      logger.error('channel:cancelFeishuQr failed', error);
      return { success: false, error: String(error) };
    }
  });

  // Forward events to renderer
  feishuLoginManager.on('qr', (data) => {
    if (!mainWindow.isDestroyed()) {
      mainWindow.webContents.send('channel:feishu-qr', data);
    }
  });

  feishuLoginManager.on('success', (data) => {
    if (!mainWindow.isDestroyed()) {
      logger.info('feishu:qr-login-success', data);
      mainWindow.webContents.send('channel:feishu-success', data);
    }
  });

  feishuLoginManager.on('error', (error) => {
    if (!mainWindow.isDestroyed()) {
      logger.error('feishu:qr-login-error', error);
      mainWindow.webContents.send('channel:feishu-error', error);
    }
  });
}
 ...
 * Stage files to ~/.keagent/media/outbound/ for gateway access
 */
 ...
 * The JSONL file lives at: ~/.keagent/agents/<agentId>/sessions/<suffix>.jsonl
