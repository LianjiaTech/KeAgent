...
import { deployAgentFrameworks } from '../utils/frameworks-deploy';
...
import { bootstrapDefaultProviders } from '../services/providers/provider-bootstrap';

const WINDOWS_APP_USER_MODEL_ID = 'app.keagent.desktop';
...
if (process.platform === 'linux') {
  app.setDesktopName('keagent.desktop');
}
...
if (!gotElectronLock) {
  console.info(
    '[ClawX] Another instance already holds the single-instance lock; exiting duplicate process'
  );
  app.exit(0);
}
...
if (gotElectronLock) {
  try {
    ...
    console.warn(
      '[ClawX] Failed to acquire process instance file lock; continuing with Electron single-instance lock only',
      error
    );
  }
}
...
async function initialize(): Promise<void> {
  ...
  // The URL filter ensures this callback only fires for gateway and Xclaude requests,
  // avoiding unnecessary overhead on every other HTTP response.
  session.defaultSession.webRequest.onHeadersReceived(
    { urls: ['http://127.0.0.1:18800/*', 'http://localhost:18800/*', 'http://*/*', 'https://*/*'] },
    (details, callback) => {
      const headers = { ...details.responseHeaders };
      delete headers['X-Frame-Options'];
      delete headers['x-frame-options'];
      // Remove frame-ancestors entirely: replacing with * is insufficient because
      // the Electron renderer loads from file:// whose origin is `null`, and
      // `frame-ancestors *` does NOT match null origins per the CSP spec.
      if (headers['Content-Security-Policy']) {
        headers['Content-Security-Policy'] = headers['Content-Security-Policy'].map((csp) =>
          csp.replace(/frame-ancestors\s+[^;]*;?\s*/g, '')
        );
      }
      if (headers['content-security-policy']) {
        headers['content-security-policy'] = headers['content-security-policy'].map((csp) =>
  ...
  // Bootstrap default Xclaude providers from VITE_DEFAULT_MODELS if no providers are configured.
  await bootstrapDefaultProviders().catch((error) => {
    logger.warn('Failed to bootstrap default providers:', error);
  });
