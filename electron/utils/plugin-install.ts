...
/**
 * Install (or configure) the official Feishu plugin using the
 * @larksuite/openclaw-lark-tools CLI.
 *
 * When `credentials` are provided the tool runs non-interactively:
 *   npx -y @larksuite/openclaw-lark-tools install --app <appId>:<appSecret> --use-existing
 *
 * When no credentials are provided it falls back to interactive mode
 * (terminal only — typically used from a setup wizard).
 */
export async function installFeishuPluginViaLarkTools(credentials?: {
  appId: string;
  appSecret: string;
}): Promise<{ installed: boolean; warning?: string }> {
  // Check if the plugin is already installed first
  const existing = ensureFeishuPluginInstalled();
  if (existing.installed) return existing;

  return new Promise((resolve) => {
    const args = ['-y', '@larksuite/openclaw-lark-tools', 'install', '--skip-version-check'];
    if (credentials) {
      args.push('--app', `${credentials.appId}:${credentials.appSecret}`);
      args.push('--use-existing');
    }

    const npxCmd = process.platform === 'win32' ? 'npx.cmd' : 'npx';
    logger.info('[plugin] Running lark-tools install', { args });
    ...
    child.on('close', (code) => {
      if (code === 0) {
        logger.info('[plugin] lark-tools install succeeded');
        resolve({ installed: true });
      } else {
        const msg = (stderr || stdout).trim().slice(0, 300);
        logger.warn('[plugin] lark-tools install failed', { code, msg });
        resolve({ installed: false, warning: `lark-tools install failed (exit ${code}): ${msg}` });
      }
    });

    child.on('error', (err) => {
      logger.warn('[plugin] lark-tools spawn error', err);
      resolve({ installed: false, warning: `Failed to spawn npx: ${err.message}` });
    });
  });
}
