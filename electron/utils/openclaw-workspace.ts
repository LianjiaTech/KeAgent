 ...
 * directories that already exist under ~/.keagent/.
 */
async function resolveAllWorkspaceDirs(): Promise<string[]> {
  ...
  // We intentionally do NOT scan ~/.keagent/ for any directory starting
    ...
    for (const file of entries) {
      ...
      if (before === '' && after === '') {
        try {
          ...
          logger.info(
            `Removed ClawX-only bootstrap file for re-seeding: ${file} (${workspaceDir})`
          );
        } catch {
...
export async function ensureClawXContext(): Promise<void> {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    logger.debug(
      `ClawX context merge: ${skipped} file(s) still missing (retry ${attempt}/${MAX_RETRIES})`
    );
  }
