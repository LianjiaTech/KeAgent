...
const LEGACY_WECHAT_SYNC_DIR = join(
  OPENCLAW_DIR,
  'agents',
  'default',
  'sessions',
...
async function ensurePluginAllowlist(
...
function transformChannelConfig(
  ...
  if (channelType === 'feishu' || channelType === 'wecom') {
    const existingDmPolicy =
      existingAccountConfig.dmPolicy === 'pairing' ? 'open' : existingAccountConfig.dmPolicy;
...
export async function saveChannelConfig(
  ...
  return withConfigLock(async () => {
    ...
    const defaultAccountData =
      accounts[mirroredAccountId] ?? accounts[resolvedAccountId] ?? accounts[DEFAULT_ACCOUNT_ID];
...
export async function listConfiguredChannelAccounts(): Promise<
  Record<string, ConfiguredChannelAccounts>
> {
  const config = await readOpenClawConfig();
  ...
  for (const [channelType, section] of Object.entries(config.channels)) {
    ...
    if (accountIds.length === 0) {
      const hasAnyPayload = Object.keys(section).some(
        (key) => !CHANNEL_TOP_LEVEL_KEYS_TO_KEEP.has(key)
      );
...
export async function setChannelDefaultAccount(
  channelType: string,
  accountId: string
): Promise<void> {
  return withConfigLock(async () => {
    ...
    if (!accounts || !accounts[trimmedAccountId]) {
      throw new Error(
        `Account "${trimmedAccountId}" is not configured for channel "${resolvedChannelType}"`
      );
    }
    ...
    await writeOpenClawConfig(currentConfig);
    logger.info('Set channel default account', {
      channelType: resolvedChannelType,
...
export async function deleteAgentChannelAccounts(
  agentId: string,
  ownedChannelAccounts?: Set<string>
): Promise<void> {
  return withConfigLock(async () => {
    const currentConfig = await readOpenClawConfig();
...
export async function cleanupDanglingWeChatPluginState(): Promise<{
  cleanedDanglingState: boolean;
}> {
  return withConfigLock(async () => {
    const currentConfig = await readOpenClawConfig();
...
export function parseDoctorValidationOutput(
  channelType: string,
  output: string
): DoctorValidationParseResult {
  const errors: string[] = [];
...
async function validateDiscordCredentials(
  ...
  try {
    ...
    if (!meResponse.ok) {
      ...
      const msg =
        (errorData as { message?: string }).message || `Discord API error: ${meResponse.status}`;
