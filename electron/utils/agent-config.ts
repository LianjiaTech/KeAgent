...
const MAIN_AGENT_ID = 'ceo';
const MAIN_AGENT_NAME = 'CEO';
const DEFAULT_ACCOUNT_ID = 'default';
const DEFAULT_WORKSPACE_PATH = '~/.keagent/workspace';
...
function getDefaultAgentDirPath(agentId: string): string {
  return `~/.keagent/agents/${agentId}/agent`;
}
...
function getManagedWorkspaceDirectory(agent: AgentListEntry): string | null {
  ...
  const configuredWorkspace = expandPath(agent.workspace || `~/.keagent/workspace-${agent.id}`);
...
export async function removeAgentWorkspaceDirectory(agent: {
  id: string;
  workspace?: string;
}): Promise<void> {
...
async function provisionAgentFilesystem(
  ...
  const mainEntry =
    entries.find((entry) => entry.id === MAIN_AGENT_ID) ?? createImplicitMainEntry(config);
  const sourceWorkspace = expandPath(mainEntry.workspace || getDefaultWorkspacePath(config));
  const targetWorkspace = expandPath(agent.workspace || `~/.keagent/workspace-${agent.id}`);
...
export async function createAgent(name: string): Promise<AgentsSnapshot> {
  return withConfigLock(async () => {
    ...
    const newAgent: AgentListEntry = {
      id: nextId,
      name: normalizedName,
      workspace: `~/.keagent/workspace-${nextId}`,
 ...
 * The template files (AGENTS.md, IDENTITY.md, SOUL.md) are copied to the agent's workspace.
 */
export async function createAgentFromTemplate(
  ...
  return withConfigLock(async () => {
    ...
    // Create workspace and agentDir
    const workspacePath = expandPath(newAgent.workspace || `~/.keagent/workspace-${nextId}`);
    const agentDirPath = expandPath(newAgent.agentDir || getDefaultAgentDirPath(nextId));
    const sessionsDir = join(getOpenClawConfigDir(), 'agents', nextId, 'sessions');

    await ensureDir(workspacePath);
    await ensureDir(agentDirPath);
    await ensureDir(sessionsDir);

    // Copy template files to workspace
    if (template.agentsMd) {
      await writeFile(join(workspacePath, 'AGENTS.md'), template.agentsMd, 'utf-8');
    }
    if (template.identityMd) {
      await writeFile(join(workspacePath, 'IDENTITY.md'), template.identityMd, 'utf-8');
    }
    if (template.soulMd) {
      await writeFile(join(workspacePath, 'SOUL.md'), template.soulMd, 'utf-8');
    }

    // Copy runtime files from main agent
    const mainEntry =
      entries.find((entry) => entry.id === MAIN_AGENT_ID) ?? createImplicitMainEntry(config);
    const sourceAgentDir = expandPath(mainEntry.agentDir || getDefaultAgentDirPath(MAIN_AGENT_ID));
    await copyRuntimeFiles(sourceAgentDir, agentDirPath);

    await writeOpenClawConfig(config);
    logger.info('Created agent from template', { agentId: nextId, templateId: template.id });
    return buildSnapshotFromConfig(config);
  });
}
  ...
  return withConfigLock(async () => {
    ...
    config.bindings = Array.isArray(config.bindings)
      ? config.bindings.filter(
          (binding) => !(isChannelBinding(binding) && binding.agentId === agentId)
        )
      : undefined;
...
export async function assignChannelAccountToAgent(
  ...
  return withConfigLock(async () => {
    ...
    await writeOpenClawConfig(config);
    logger.info('Assigned channel account to agent', {
      agentId,
      channelType,
