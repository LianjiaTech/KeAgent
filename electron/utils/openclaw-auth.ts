...
function getAuthProfilesPath(agentId = 'ceo'): string {
  return join(getOpenClawConfigDir(), 'agents', agentId, 'agent', AUTH_PROFILE_FILENAME);
}

async function readAuthProfiles(agentId = 'ceo'): Promise<AuthProfilesStore> {
...
async function writeAuthProfiles(store: AuthProfilesStore, agentId = 'ceo'): Promise<void> {
...
async function discoverAgentIds(): Promise<string[]> {
  ...
  try {
    if (!(await fileExists(agentsDir))) return ['ceo'];
...
export async function saveOAuthTokenToOpenClaw(
  ...
  if (agentIds.length === 0) agentIds.push('ceo');
 ...
 * @param agentId - Optional single agent ID to read from, defaults to 'ceo'
