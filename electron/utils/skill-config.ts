 ...
 * Direct read/write access to skill configuration in ~/.keagent/openclaw.json
...
import { getOpenClawDir, getResourcesDir, getOpenClawConfigDir } from './paths';
 ...
 * ~/.keagent/skills/ on first launch.  These come from the openclaw package's
 * extensions directory and are available in both dev and packaged builds.
 */
const BUILTIN_SKILLS = [] as const;

/**
 * Ensure built-in skills are deployed to ~/.keagent/skills/<slug>/.
 ...
 * deployed to ~/.keagent/skills/<slug>/ as full directories.
 *
 * Policy:
